...
resample.fun = function(job, data, i) {
  if (is.numeric(data$task))
    data$task = OpenML::convertOMLDataSetToMlr(OpenML::getOMLDataSet(data$task))
  ...
  for (p in problems) {
    ...
    if (nrow(exps) > 0) {
      ...
      if (is.numeric(problem$data$task)) {
        problem$data$task = OpenML::convertOMLDataSetToMlr(OpenML::getOMLDataSet(problem$data$task))
